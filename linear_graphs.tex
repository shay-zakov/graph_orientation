...
\begin{algorithm}
	...
	\For{$j=1$ to $n$}{
		\While{$i^r < j-1$ and $H^\ell_s(k^r_{i^r, j}) \leq \max\left\{H^\ell_s(i^r), h_s(\vec{P}_{i^r, j})\right\}$}{
			$i^r \gets k^r_{i^r, j}$\;
		}
		$H^{r}[j] \gets \max \{ H^{\ell}[i^r], h_s(\vec{P}_{i^r, j})\}$\;
		\While{$i^\ell < j-1$ and $H^r_s(k^\ell_{i^\ell, j}) \leq \max\left\{H^r_s(i^\ell), h_s(\cev{P}_{i^\ell, j})\right\}$}{
			$i^\ell \gets k^\ell_{i^\ell, j}$\;
		}
