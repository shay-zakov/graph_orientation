...
%Then $S_j$ has the property that 
%for $s_i \in S_j$
%$\mu_{i}=\min_{1\leq k \leq j} \{value_k \mid key_k \geq key_{i}\}$,
%see Proposition \ref{p.kmu}.  
...
%		  	\end{itemize}\ 
%	  	\{\emph{comment: $\mu_{k-1}<value< \mu_{p}$ }\}
%		  	\item let $\mu_k$ be largest in $S_{j-1}$ such that  $\mu_k<value  $;
%		  	\item if $k+1 \leq m-1$ delete from $S_{j-1}$ all pairs $s_i$, $k+1\leq i \leq m-1$ \{\emph{comment: $\mu_{i}\geq value  $ for all such i}\} ;
...
\begin{theorem}\label{p.kmu} The updating procedure preserves the following invariants of $S_j$.
  \begin{enumerate}
   ...
   Taken together the two invariants imply the correctness of the $\min_j$ computation: if $key_{i-1}<W\leq key_{i}$ for some $1\leq i \leq \ell_j+1$. then 
  	$\min_j(W)=value_{i}$.
%Moreover, the overall running time for solving Problem \ref{p.msp} is $O(n \log n)$.
\end{theorem}
\begin{proof}
