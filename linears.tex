...
\KwOut{an optimal orientation of $L$ under $H_x$}
	...
	\For{$j=1$ to $n$}{
		set $H^{\succ}(j)$ to the minimum over $0\leq i <j$ of $\max \{ H^{\prec}(i), h_x(\overto{L}_{i, j})\}$\;
		\label{st.1}
		set $H^{\prec}(j)$ to the minimum over $0\leq i <j$ of  $\max \{ H^{\succ}(i), h_x(\overfrom{L}_{i, j})\}$\;
			\label{st.2}
	}
...
\begin{problem}[OCMM - Online Column Minima of  a c-Monotone matrix]\label{p.ocmm}
		For $1\leq j \leq n$ compute $H(j)=\min \{M(i,j) \mid 0\leq i \leq n\}$, where 
		$M$ is c-monotone and
	the values of $H(j'),\ 1\leq j'<j$ have to be computed before $M(i,j)$ can be evaluated.
\end{problem}
