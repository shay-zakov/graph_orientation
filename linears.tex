...
\begin{algorithm}
	...
	\For{$j=1$ to $n$}{
		set $H^{\succ}[j]$ to the minimum over $0\leq i <j$ of $\max \{ H^{\prec}[i], h_x(\vec{P}_{i, j})\}$\;
		\label{st.1}
		set $H^{\prec}[j]$ to the minimum over $0\leq i <j$ of  $\max \{ H^{\succ}[i], h_x(\cev{P}_{i, j})\}$\;
			\label{st.2}
	}
...
\subsection{Running time of \emph{BestCostPath-s}}
...
\end{enumerate}
%In addition we know from equation (\ref{eq.hsij}) that  $g[i,j]$, 
%which is either $h_s(\vec{P}_{i, j})$ or $h_s(\cev{P}_{i, j})$, is non-increasing in $i$,
%	$g[i, j]\geq g[i+1, j], 0\leq i \leq j$, and non-decreasing in $j$,
%	$g[i, j]\leq g[i, j+1], 0\leq j \leq n-1$.
